...
interface RunData {
  ...
  result: GenerationResult | null; error: string | null;
}
...
export default function ResultPage() {
  ...
  const { data, isLoading, error } = useQuery<RunData>({
    ...
    queryFn: async () => { const res = await apiRequest("GET", `/api/runs/${id}`); return res.json(); },
    refetchInterval: (query) => {
      const d = query.state.data;
      if (!d) return 2000;
      return (d.status === "completed" || d.status === "error") ? false : 2000;
    },
  });
  ...
  const handleOpenTermSheetFullScreen = useCallback(() => {
    if (!result) return; openInNewTab(result.termSheetHtml);
  }, [result]);
    ...
    <div className="min-h-screen flex flex-col" style={{ background: "#f7f9fc" }}>
          ...
          {isGenerating && (
            <div className="flex flex-col items-center justify-center py-24 gap-5" data-testid="status-generating">
              ...
              <div className="text-center space-y-1.5">
                <p className="text-sm font-semibold text-gray-800">Generating documents for {data?.target}...</p>
                <p className="text-xs text-gray-500">Researching country context and building interactive term sheet &amp; pitch deck</p>
              </div>
            </div>
          )}
            ...
            <div className="space-y-6 animate-in fade-in duration-500" data-testid="status-completed">
              <div className="flex items-center justify-between flex-wrap gap-4">
                ...
                <div className="flex gap-2 flex-wrap">
                  <Button variant="outline" size="sm" onClick={handleDownloadDocx} className="text-xs border-gray-200 text-gray-600" data-testid="button-download-docx">
                    <FileDown className="h-3.5 w-3.5 mr-1.5" /> .docx
                  </Button>
                  <Button variant="outline" size="sm" onClick={handleDownloadTermSheet} className="text-xs border-gray-200 text-gray-600" data-testid="button-download-termsheet">
                    <Download className="h-3.5 w-3.5 mr-1.5" /> Term Sheet
                  </Button>
                  <Button variant="outline" size="sm" onClick={handleDownloadPitchDeck} className="text-xs border-gray-200 text-gray-600" data-testid="button-download-pitchdeck">
              ...
              <div className="rounded-xl p-4 flex items-center justify-between gap-4"
                ...
                <Button size="sm" onClick={handleOpenTermSheetFullScreen} data-testid="button-open-interactive"
                  className="font-semibold bg-white hover:bg-gray-50" style={{ color: "#0000E5" }}>
              ...
              <Tabs value={activeTab} onValueChange={setActiveTab}>
                <div className="flex items-center justify-between mb-3">
                  <TabsList className="bg-white border border-gray-200">
                    <TabsTrigger value="termsheet" className="text-xs gap-1.5" data-testid="tab-termsheet"><FileText className="h-3.5 w-3.5" /> Term Sheet</TabsTrigger>
                    <TabsTrigger value="pitchdeck" className="text-xs gap-1.5" data-testid="tab-pitchdeck"><Presentation className="h-3.5 w-3.5" /> Pitch Deck</TabsTrigger>
                  </TabsList>
                  <Button variant="ghost" size="sm" className="text-xs text-gray-500" data-testid="button-open-new-tab"
                    onClick={() => openInNewTab(activeTab === "termsheet" ? result.termSheetHtml : result.pitchDeckHtml)}>
                ...
                <TabsContent value="termsheet">
                  <Card className="overflow-hidden bg-white border-gray-200"><CardContent className="p-0">
                    <iframe srcDoc={result.termSheetHtml} className="w-full border-0" style={{ height: "800px" }} title="Term Sheet" data-testid="iframe-termsheet" />
                  </CardContent></Card>
                </TabsContent>
                <TabsContent value="pitchdeck">
                  <Card className="overflow-hidden bg-white border-gray-200"><CardContent className="p-0">
                    <iframe srcDoc={result.pitchDeckHtml} className="w-full border-0" style={{ height: "800px" }} title="Pitch Deck" data-testid="iframe-pitchdeck" />
                  </CardContent></Card>
                </TabsContent>
              </Tabs>

              <div className="text-center py-2">
                <Button variant="outline" size="sm" onClick={() => navigate("/")} className="text-xs border-gray-200 text-gray-600" data-testid="button-new-search">
      ...
      <footer className="py-5 px-6" style={{ borderTop: "1px solid #e8ecf1" }}>
        <div className="max-w-6xl mx-auto flex items-center justify-between gap-4">
          ...
          <p className="text-[11px] text-gray-400 whitespace-nowrap">© 2026. Confidential &amp; Proprietary.</p>
